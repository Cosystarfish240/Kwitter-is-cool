      ...
      firebase.database().ref("/" + room_name).on('value', function (snapshot) {
            ...
            snapshot.forEach(function (childSnapshot) {
                  ...
                  if (childKey != "purpose") {
                        ...
                        span_with_distag = "<span class='glyphicon glyphicon-thumbs-down'>dislike: " + dislike + "</span></button><hr>";
                        row = name_with_tag + message_with_tag + like_button +  span_with_tag + dislike_button + span_with_distag;
...
function updateDislike(message_id)
{
      ...
      button_id = message_id;
      likes = document.getElementById(button_id).value;
      updated_likes = Number(dislike) + 1;
      console.log(updated_likes);

      firebase.database().ref(room_name).child(message_id).update({
            dislike : updated_likes
      });
}
function logout() {
      localStorage.removeItem("user_name");
